...
string xorings(string a,string b){
	string res = "";
...
void hashing(string bmsg,string hmsg,int n){
	...
	for(int i=0;i<m;i++)
		comp_box[i] = i;
	string hashed = "",compressed = "",temp = "";
	for(int i = 0;i < len;i += n){
		if(n == m){
			hashed = feistel_cipher(bmsg.substr(i,n),hmsg);
			hmsg = xorings(hashed,hmsg);
            hmsg = xorings(hmsg,bmsg.substr(i,n));
			hashed = "";
		}
		else{
			compressed = compress(bmsg,i,comp_box,m);
			hashed = feistel_cipher(compressed,hmsg);
			hmsg = xorings(hashed,hmsg);
            hmsg = xorings(hmsg,compressed);
			hashed = "";
		}
	}
